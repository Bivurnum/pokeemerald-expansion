...
static bool8 TryDoMapTransition(void)
{
    ...
    if (MPS_ENABLE_MAP_PREVIEWS && GetLastUsedWarpMapSectionId() != gMapHeader.regionMapSectionId && (MapHasPreviewScreen(gMapHeader.regionMapSectionId, MPS_TYPE_CAVE) == TRUE || MapHasPreviewScreen(gMapHeader.regionMapSectionId, MPS_TYPE_BASIC) == TRUE))
    {
