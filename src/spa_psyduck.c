...
#define sPsyduckHeadLeftSpriteId    sSpaData.monSpriteIds[0]
#define sPsyduckHeadRightSpriteId   sSpaData.monSpriteIds[1]
#define sPsyduckBillSpriteId        sSpaData.monSpriteIds[2]
#define sPsyduckHairSpriteId        sSpaData.monSpriteIds[3]
#define sPsyduckEyesSpriteId        sSpaData.monSpriteIds[4]
#define sPsyduckBodyLeftSpriteId    sSpaData.monSpriteIds[5]
#define sPsyduckBodyRightSpriteId   sSpaData.monSpriteIds[6]
#define sPsyduckTailSpriteId        sSpaData.monSpriteIds[7]
#define sPsyduckFootSpriteId        sSpaData.monSpriteIds[8]
#define sPsyduckArmFrontSpriteId    sSpaData.monSpriteIds[9]
#define sPsyduckArmBackSpriteId     sSpaData.monSpriteIds[10]
...
void CreatePsyduckSprites(u8 taskId)
{
    ...
    else
    {
        ...
        for (i = 0; i < MAX_BUGS; i++)
        {
            ...
            StartSpriteAnim(&gSprites[sSpaData.bugSpriteIds[i]], gSprites[sSpaData.bugSpriteIds[i]].sBugDirection);
        }
    }
}
...
static void SpriteCB_Bug(struct Sprite *sprite)
{
    ...
    {
        u16 HoneyX = gSprites[sSpaData.honeySpriteId].x;
        u16 HoneyY = gSprites[sSpaData.honeySpriteId].y;

        if ((sprite->x > (HoneyX - 8) && sprite->x < (HoneyX + 8))
         && (sprite->y > (HoneyY - 8) && sprite->y < (HoneyY + 8)))
        {
            u8 honeyAnimNum = gSprites[sSpaData.honeySpriteId].animNum + 1;
    ...
    if (sprite->sBugInterval == sprite->sInterval)
    {
        sprite->sBugInterval = 0;
        sprite->sInterval = (Random32() % (BUG_MAX_MOVE - BUG_MIN_MOVE)) + BUG_MIN_MOVE;
